...
check("with a way into the walkthrough", !!$("sheet").querySelector('[data-act="tour"]'));
click("#scrim");
...
check("rows without a meaningful best are left unticked",
...
// comparison already on screen, so nothing navigates away.
{
  ...
  const unmarked = rows.filter(r => RANKED.includes(r.querySelector(".rowlab").textContent.trim())
                       ...
                       .map(r => r.querySelector(".rowlab").textContent.trim());
...
head("THE WALKTHROUGH");
click('[data-tab="home"]');
// The intro card was dismissed earlier, so start it the way a returning user would.
click('[data-act="guide"]');
click('#sheet [data-act="tour"]');
check("starting it puts a coach mark on screen", !!$("coach"));
check("it spotlights a real element", !!q("#coach .spot"));
check("with an arrow and a bubble", !!q("#coach .arw") && !!q("#coach .bub"));
check("numbered so you know how long it is", /STEP 1 OF 5/.test($("coach").textContent));
check("and it points at the decision card first", txt().includes("decisions waiting"));
check("skip is offered from the first step", !!q('[data-act="tourskip"]'));
click('[data-act="tournext"]');
check("next advances the step", /STEP 2 OF 5/.test($("coach").textContent));
check("and moves to the screen that step needs", txt().includes("Wishlist"));
click('[data-act="tournext"]');
click('[data-act="tournext"]');
check("step 4 lands on the comparison", !!q("#screen .cmp"), txt().slice(0, 50));
check("having set a real set up", all("#screen .cmp thead th").length > 2);
click('[data-act="tournext"]');
check("the last step reads Done", $("coach").textContent.includes("Done"));
click('[data-act="tournext"]');
check("finishing removes the overlay", !$("coach"));
click('[data-tab="home"]');
click('[data-act="guide"]');
click('#sheet [data-act="tour"]');
click('[data-act="tourskip"]');
check("skip ends it too", !$("coach"));
check("and says you can come back", !!q(".toast"));
